...
def indent_score(relative_angles):
    ...
    shape = np.zeros((peak[0] + length - peak[1]))
...
def my_find_corner_signature(cnt):
    ...
    while sigma <= max_sigma:
        ...
        relative_angles = get_relative_angles(np.array(cnt_convert), sigma=sigma)
...
def export_contours(img, img_bw, contours, path, modulo):
    ...
    cv2.imwrite("../images/output/color_border.png", out_color)
